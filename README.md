...
Untrusted input is a plan output; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a plan output
- **Closed type:** `type Decision = Apply(ChangeSet) | Reject` where `ChangeSet` is closed
- **Sink / capability:** `grant applyPlan irreversible` → `commit { applyPlan(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/iac-apply-gate/demo.sh
```
...
- `iac-apply-gate_safe.eide` — the correct design.
- `iac-apply-gate_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
